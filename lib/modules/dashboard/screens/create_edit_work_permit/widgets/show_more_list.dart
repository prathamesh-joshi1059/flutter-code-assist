...
class ShowMoreListWidget extends StatefulWidget {
  final List selectedDataList;
  final bool isPPEList;

  const ShowMoreListWidget({
    super.key,
    this.selectedDataList = const [],
    this.isPPEList = false,
  });

  @override
  State<ShowMoreListWidget> createState() => _ShowMoreListWidgetState();
}

class _ShowMoreListWidgetState extends State<ShowMoreListWidget> {
  ValueNotifier showMoreLessNotifier = ValueNotifier(true);
  List allDetailsList = [];

  initData() {
    // widget.initialCheckList.sort((a, b) => (a['name']).compareTo(b['name']));
    allDetailsList = widget.selectedDataList;
  }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(width: 1.w),
          SizedBox(
            width: (widget.isPPEList) ? 76.w : 80.w,
            child: TextWidget(
